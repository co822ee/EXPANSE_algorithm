...
2. Use the IPCC5 climate classifications
3. Improvement the algorithms (for multiple years):

* SLR: Mixed effect model
* GWR: add time component (GTWR)
* RF: add climate zones and years as predictors (?)

4. Improve the algorithms (for a single year):

* SLR: after predictors are selected, climate zones are included as interaction terms to further improve predictions.
* GWR: check other kernel functions 
...
```{r read in gwr, fig.width=9, fig.height=7}
...
for(i in seq_along(csv_names)){
   ...
   grd <- setup[[2]]
   DM <- setup[[3]]
   gwr_model <- gwr(sp_train, grd, DM, nngbs[i], csv_names[i])
   fun <- function() {
      plot(eu_bnd[1], add = TRUE,col='transparent',border='grey', alpha=0.05)
   }
   print(csv_names[i])
   print(read.csv(paste0("data/workingData/SLR_summary_model_", csv_names[i], '.csv'))[,c(1:2,7)])
   print(paste0("nearest neighbours: ", nngbs[i]))
