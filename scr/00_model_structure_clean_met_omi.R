...
# Multiple years:
csv_names <- paste0('run2_met_omi_',c('08-10', '09-11', '10-12', '08-12'))   #2008:2012
...
foreach(i = seq_along(csv_names)) %dopar% {
   ...
   omi <- read.csv("../EXPANSE_predictor/data/processed/no2_2010_elapse_climate_omi.csv", header=T)
   omi <- omi %>% dplyr::select("station_european_code", matches("omi"))
   omi_sub <- omi %>% dplyr::select(paste0('omi_',years[[i]]),"station_european_code")
   omi_sub_l <- lapply(years[[i]], function(yr){
      return(omi_sub %>% dplyr::select("station_european_code", paste0('omi_', yr)) %>% 
                rename(omi=paste0('omi_', yr)) %>% mutate(year=yr))
   })
   sapply(omi_sub_l, names)
   lapply(omi_sub_l, function(l) l$year %>% unique)
   omi_sub <- do.call(rbind, omi_sub_l)
   if(length(years[[i]]>1)){
   ...
   data_all0 <- inner_join(data_all, met %>% dplyr::select(-system.index, -.geo), by=c("station_european_code", "year"))
   data_all <- inner_join(data_all0, omi_sub, by=c('station_european_code', 'year'))
   ...
   pred_c <- c(pred_c, "pressure", "temp",
               "precip", "wind_speed", "omi")  #"v_wind", "u_wind"
