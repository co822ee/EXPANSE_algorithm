...
foreach(i=seq_len(nrow(comb))) %dopar% {
   ...
   print(csv_name)
   no2_e_target <- subset_df_yrs(no2_e_all, years[[i]])
   ...
   sum(train_sub$type_of_st=="Background"&train_sub$zoneID==1)/nrow(train_sub)
   sum(test_sub$type_of_st=="Background"&test_sub$zoneID==1)/nrow(test_sub)
   ...
   # --> FALSE: yes indeed the stations are not the same in training and test data
   ...
   slr_poll <- read.csv(paste0('data/workingData/SLR_result_all_run2_', years[[i]],".csv"))
   ...
   if(!(typeof(gwr_model_ad)=='logical')){
      ...
      em <- output_em(gwr_df_ad, paste0(csv_name, '_ad'), 'gwr', years[[i]])
