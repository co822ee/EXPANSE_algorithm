...
csv_names <- paste0('NO2_5cv_omi_', c(2005:2012))
...
x_varname = names(subset_df_yrs(no2_e_all, years[[1]]) %>% dplyr::select(matches(pred_c_rf)))
...
files <- list.files("data/workingData/", "SLR_summary_model_run2_omi_")[grepl("fold", list.files("data/workingData/", "SLR_summary_model_run2_omi_"))]
comp_rf_slr <- function(csv_i){
   slr_name <- files[csv_i]
   yr <- strsplit(slr_name, "_")[[1]][6] %>% as.numeric()
   ...
   p2 <- plot_rf_vi(paste0("run2_omi_", yr, "_fold_", nfold), var_no = 10)
...
#    perfm <- lapply(paste0("data/workingData/", list.files("data/workingData/", paste0("perf_m_run2_omi_", years[[i]]))), read.csv)
