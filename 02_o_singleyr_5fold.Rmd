...
csv_names <- paste0('o_gwrid_NO2_5cv_', c(2006:2012))
...
comp_rf_slr <- function(csv_i){
   slr_name <- list.files("data/workingData/", "SLR_summary_model_o_gwrid_")[csv_i]
   ...
   p2 <- plot_rf_vi(paste0("o_gwrid_", yr, "_fold_", nfold), var_no = 10)
   
   var_importance <- read.csv(paste0('data/workingData/GWR_rf_vi_o_gwrid_', yr, '_fold_', nfold, '.csv'), 
                              header = T)
...
# comp_rf_slr(4)
lapply(seq_along(list.files("data/workingData/", "SLR_summary_model_o_gwrid_")), comp_rf_slr)
```
